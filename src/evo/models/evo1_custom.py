...
from src.evo.models.components.vision_encoders.vlaser_embedder import VlaserEmbedder
from src.evo.models.components.action_heads.flow_matching import FlowmatchingActionHead
from types import SimpleNamespace

class EVO1Custom(nn.Module):
    def __init__(self, config: Union[str, Path, dict]):
        ...
        vlm_name = model_cfg.get("vision_encoder", "OpenGVLab/InternVL3-2B") 
        self.embedder = VlaserEmbedder(model_name=vlm_name, device=self._device)
    ...
    def set_finetune_flags(self):
        ...
        if not train_cfg.get("finetune_vlm", False):
            self._freeze_module(self.embedder, "VLM (Vlaser)")
